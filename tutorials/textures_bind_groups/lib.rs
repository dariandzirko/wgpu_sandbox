...
const INDECES: &[u16] = &[0, 1, 4, 1, 2, 4, 2, 3, 4];
...
impl State {
    // Creating some of the wpgu types requires async code
    async fn new(window: Window) -> Self {
        ...
        // # Safety
        //
        // The surface needs to live as long as window that created it.
        ...
        let diffuse_texture = device.create_texture(&wgpu::TextureDescriptor {
            // All textures are stored as 3D, we represent out 2D texture
...
pub async fn run() {
    ...
    // State::new uses async code, so we're going to have to wait for it to finish
    ...
    event_loop.run(move |event, _, control_flow| {
        match event {
            ...
            Event::RedrawRequested(window_id) if window_id == state.window().id() => {
                ...
                match state.render() {
                    Ok(_) => {}
                    // Reconfigure the surface if it's lost or outdate
            ...
            Event::RedrawEventsCleared => {
                // RedrawReguested will trigger only once we manually
                // request it
